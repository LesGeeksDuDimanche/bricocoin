...
#if defined(NDEBUG)
# error "Bricocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe5f177c73aa22ecc6fbe8616d7c1e89c1c86d31c75d4be0f1b807bf8a2b91f74");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bricocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bricocoin Signed Message:\n";
...
{
    // Bricocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10); // Bricocoin: 10 blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.007 * 24 * 60 * 60; // Bricocoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Bricocoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bricocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bricocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0447f3dadede10c1f2c13666196a241aa84786eea62513084cb481e8f827a5abfbc1e941f946ef10bf2b99d8f96de3d2640e8b093dd641c2cdeb70dea30d8c6fa4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1516556306;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084609181;
...
if (false && block.GetHash() != hashGenesisBlock)
{
  ...
  printf("block.GetHash = %s\n", block.GetHash().ToString().c_str());
        ...
        assert(block.hashMerkleRoot == uint256("0xee96e2d1613cc9d444d5455ea1788293e91220e61b57a602665d3c325ad2ca60"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BricocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BricocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BricocoinMiner(CWallet *pwallet)
{
    printf("BricocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bricocoin-miner");
    ...
    try { loop {
        ...
        printf("Running BricocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BricocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BricocoinMiner, pwallet));
}
